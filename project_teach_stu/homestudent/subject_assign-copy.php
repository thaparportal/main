...
?></span></div>
			...
			<div class="content-wrap">
				<div class="content">
					<header class="c-header">
						...
						<h1><?php echo $sname."<span> $tname $purpose</span>"; 
	...
	if($tlp!='l')
	{
		?><table>
					...
					<tbody>
						<?php
    $assignment="SELECT * from `assignment_questions` where `subject-code`='$scode' and `group`='$group' and tlp='$tlp' and teacher_code='$tcode'";
    $res=mysqli_query($con,$assignment);
    $url='subject_assign_upload.php';
    while($sub=mysqli_fetch_array($res))
    {   
        $aname=$sub['assignment_name'];
        $gtime=$sub['time_given'];
        $ltime=$sub['Last_date'];
        $aname_link= "<a href='$url?group=$group&scode=$scode&tlp=$tlp&tcode=$tcode'>$aname</a>";
        $gtime_link= "<a href='$url?scode=$scode & tlp=$purpose&tname=$tname'>$gtime</a>";
        $ltime_link= "<a href='$url?scode=$scode & tlp=$purpose&tname=$tname'>$ltime</a>";
    ...
    {   
        ...
        $gtime=$sub['time_given'];
        $aname_link= "<a href='$url?scode=$scode&tlp=$purpose&tname=$tcode'>$sname</a>";
        $gtime_link= "<a href='$url?scode=$scode&tlp=$purpose&tname=$tcode'>$gtime</a>";        
